...
for i, jetpt in enumerate(jet_pt_l):
  htemp = TH1D('htemp',f'Fragmentation function of {HFcand} in jets', 110, 0, 1.1)
  treeSimu.Draw("z>>htemp",f'pt_cand>{pt_cand_l[i]}&&pt_cand<{pt_cand_u[i]}&&pt_jet>{jet_pt_l[i]}&&pt_jet<{jet_pt_u[i]}&&{jet_selection}')
